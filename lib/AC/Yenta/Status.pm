...
#
# $Id: Status.pm,v 1.38 2010/10/30 19:46:16 jaw Exp $
...
sub init {
    ...
    AC::DC::Sched->new(
        info	=> 'save status',
        freq	=> (conf_value('time_status_save') || 15),
        func	=> \&save_status,
       );
}
...
sub periodic {
    ...
    __PACKAGE__->isdown($id) unless $c;
...
sub _random_peer {
    ...
    # sceptical
    my @peer = values %{$DATA->{sceptical}};

    # known peer
    unless( @peer ){
        @peer = map { $DATA->{allpeer}{$_} } keys %{$DATA->{peertype}{yenta}};
    }
    ...
    if( @peer ){
        ...
        return ($p->{server_id}, $p->{ip}, undef)
          if int rand(@peer+1);	# sometimes use the seed
    }
...
sub update_sceptical {
    ...
    debug("rcvd update (sceptical) about $id");
    $up->{id} = $id;
