...
#
# $Id: Merkle.pm,v 1.10 2010/12/23 19:09:39 jaw Exp $
...
sub _merkle_update {
    ...
    unless( $lev ){
        ...
        $db->put($me->{name}, 'merkle', 'root', $hash);
        return;
    }

    # get node
    my $d = $db->get($me->{name}, 'merkle', $k1);
